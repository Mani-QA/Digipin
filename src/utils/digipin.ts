...
export function getLatLngFromDigiPin(digiPin: string): LatLng {
...
export function digiPinToCoordinates(digiPin: string): { latitude: number; longitude: number } {
  const pin = digiPin.replace(/-/g, '');
  if (pin.length !== 10) {
    throw new Error('Invalid DIGIPIN format. Expected 10 characters.');
  }
...
export function coordinatesToDigiPin(latitude: number, longitude: number): string {
